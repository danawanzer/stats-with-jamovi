...
The repeated measures analysis of variance (ANOVA) is used to test the difference in our dependent variable between [three or more]{.ul} groups of observations in which all participants participate in all groups or levels. Our grouping variable is our independent variable. In other words, we use the repeated measures ANOVA when we have a research question with a **continuous dependent variable** and a **categorical independent variable with three or more categories in which the [same]{.ul} participants are in each category**.
...
We check assumptions using the "Assumption Checks" tab under the repeated measures ANOVA function. To test normality, we select Q-Q plot. It is currently the only option we have available for testing normality in the repeated measures ANOVA.
...
2.  Under "Repeated Measures Factors" name your independent variable and rename the levels to match the repeated measurements. In this case you can name it "Task". Rename the three levels of Task: Speech, Conceptual, and Syntax.
...
5.  In the Assumption Checks drop-down menu, select `Sphericity tests` and `Q-Q Plots`. You'll note that if you violate the assumption of sphericity, there are two corrections provided (see [testing sphericity]). Uncheck "none" if you fail to meet the assumption so you only perform one test.
...
Similar to the dependent t-test, I am not a fan of the output for the repeated measures ANOVA in jamovi under "estimated marginal means" but it's good enough for our purposes. Be sure to select "observed scores" under plot.
