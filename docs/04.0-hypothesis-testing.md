...
<img src="04.0-hypothesis-testing_files/figure-html/unnamed-chunk-1-1.png" alt="**CAPTION THIS FIGURE!!**" width="100%" />
...
We then go to a [*t-*value table like this one](http://www.z-table.com/t-value-table.html) and find the cell we are looking for. First, our one-tailed probability is .05 so we're going to look under the sixth column (t~.95~, one-tail = .05, two-tails = .10). Then we need to find the row for our degrees of freedom (df = 28). That leads us to a *t*-score of **1.701**.

Therefore, we can now finalize this step: our criteria for decision is as such:

-   *t* \> 1.701 means we *reject the null hypothesis*

-   *t* \<= 1.701 means we *fail to reject the null hypothesis*
