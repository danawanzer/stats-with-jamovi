...
```{block, type = "warning"}
Do not use the one-way ANOVA analysis in jamovi! The options there are too limited for our use. Instead, be sure you use ANOVA!
```
...
```{r echo = FALSE, fig.cap = "One-way ANOVA setup in jamovi", out.width = "80%"}
knitr::include_graphics("images/04_one-way-anova/one-way_setup.png")
```
...
```{r echo = FALSE, fig.cap = "Testing assumptions in jamovi"}
knitr::include_graphics("images/04_one-way-anova/one-way_assumptions1.png")
```

```{r echo = FALSE, fig.cap = "Testing assumptions in jamovi"}
knitr::include_graphics("images/04_one-way-anova/one-way_assumptions2.png")
```
...
knitr::include_graphics("images/04_one-way-anova/one-way_results.png")
```
...
```{r echo = FALSE, fig.cap = "One-way ANOVA results in jamovi"}
knitr::include_graphics("images/04_one-way-anova/one-way_results_Welch.png")
```
...
```{r echo = FALSE, fig.cap = "Kruskal-Wallis results in jamovi"}
knitr::include_graphics("images/04_one-way-anova/one-way_results_Kruskal.png")
```
...
```{r echo = FALSE, fig.cap = "Contrasts - Deviation"}
knitr::include_graphics("images/04_one-way-anova/contrasts_deviation.png")
```
...
```{r echo = FALSE, fig.cap = "Contrasts - Simple"}
knitr::include_graphics("images/04_one-way-anova/contrasts_simple.png")
```

3.  **Difference**: Each category (except the first) is compared to the mean effect of all previous categories.

```{r echo = FALSE, fig.cap = "Contrasts - Difference"}
knitr::include_graphics("images/04_one-way-anova/contrasts_difference.png")
```

4.  **Helmert**: Each category (except the last) is compared to the mean effect of all subsequent categories.

```{r echo = FALSE, fig.cap = "Contrasts - Helmert"}
knitr::include_graphics("images/04_one-way-anova/contrasts_helmert.png")
```

5.  **Repeated**: Each category is compared to the last category.

```{r echo = FALSE, fig.cap = "Contrasts - Repeated"}
knitr::include_graphics("images/04_one-way-anova/contrasts_repeated.png")
```
...
```{r echo = FALSE, fig.cap = "Contrasts - Polynomial"}
knitr::include_graphics("images/04_one-way-anova/contrasts_polynomial.png")
```

**Test yourself!** Which contrast would make [most sense]{.ul} to test given that we want to know how our drug compares to the other two drugs? `r mcq(c("deviation", "simple", "difference", "helmert", answer = "repeated", "polynomial"))`
...
```{r echo = FALSE, fig.cap = "Post hoc test results in jamovi"}
knitr::include_graphics("images/04_one-way-anova/one-way_results_post-hoc.png")
```
...
1.  **Does students differ on English scores by rank (i.e., freshmen, sophomore, junior, senior)?**
    ...
    -   Do you satisfy the assumption of normality? `r mcq(c(answer = "yes", "no"))`

    -   Do you satisfy the assumption of homogeneity of variance? `r mcq(c(answer = "yes", "no"))`

    -   Which statistic should you use? `r mcq(c(answer = "one-way ANOVA", "Welch's F-test", "Kruskal-Wallis test"))`

    -   Do students differ on English scores by rank? `r mcq(c("yes", answer = "no"))`

    -   Should you perform a planned contrast or post hoc comparison? `r mcq(c("yes", answer = "no"))`

    -   What are the results of the post hoc comparison? `r mcq(c(answer = "N/A - Don't perform", "Freshmen had higher English scores than sophomores, juniors, and seniors", "Freshmen and sophomores had higher English scores than juniors and seniors"))`

2.  **Does smoking status (Smoking: Nonsmoker = 0, Past smoker = 1, Current smoker = 2) relate to sprint time?**

    -   Do you satisfy the assumption of normality? `r mcq(c("yes", answer = "no"))`

    -   Do you satisfy the assumption of homogeneity of variance? `r mcq(c(answer = "yes", "no"))`

    -   Which statistic should you use? `r mcq(c("one-way ANOVA", "Welch's F-test", answer = "Kruskal-Wallis test"))`

    -   Does smoking status relate to sprint time? `r mcq(c(answer = "yes", "no"))`

    -   Should you perform a planned contrast or post hoc comparison? `r mcq(c(answer = "yes", "no"))`

    -   What are the results of the post hoc comparison? `r mcq(c("N/A - Don't perform", answer = "Nonsmokers had significantly faster sprint times than current smokers", "Nonsmokers and past smokers had significantly faster spring times than current smokers", "Nonsmokers had significantly faster sprint times than both past and current smokers"))`
